...
# the lowercase database name (e.g., . test), connect to CDB using 'sq', connect to each PDB,
# and output only PDB name and used size (in GB) in a single-node RAC database.
# Includes debugging to diagnose no output issues.
# Assumptions:
# - Run as the oracle user with access to sq (alias/script for sqlplus / as sysdba).
# - Sourcing the lowercase database name (e.g., . test) sets ORACLE_SID, ORACLE_HOME, and PATH.
# - Databases and instances are running; offline databases/PDBs are skipped.
...
# Debug: Log the database names found
echo "DEBUG: Databases found: $dbs" >&2

if [ -z "$dbs" ]; then
  echo "DEBUG: No databases found in pmon process list (excluding asm*, apx*, mgmtdb)." >&2
  exit 1
fi

# Loop through each database
for db in $dbs; do
  # Debug: Log the database being processed
  echo "DEBUG: Processing database: $db" >&2
  ...
  # Check if sq is available
  if ! command -v sq >/dev/null 2>&1; then
    echo "DEBUG: Database $db: sq command not found." >&2
    continue
  fi
  
  # Debug: Log ORACLE_SID and ORACLE_HOME
  echo "DEBUG: Database $db: ORACLE_SID=$ORACLE_SID, ORACLE_HOME=$ORACLE_HOME" >&2
...
EOF
  ...
  if [ $? -ne 0 ] || [ -z "$pdbs" ]; then
    echo "DEBUG: Database $db: No PDBs found or not a CDB (exit code: $?)." >&2
    continue
  fi

  # Debug: Log PDBs found
  echo "DEBUG: Database $db: PDBs found: $pdbs" >&2
...
EOF
    ...
    if [ $? -ne 0 ]; then
      echo "DEBUG: Database $db, PDB $pdb: Failed to retrieve size (exit code: $?)." >&2
      continue
    fi

    # Debug: Log size retrieved
    echo "DEBUG: Database $db, PDB $pdb: Used size: $used_size" >&2
...
# the lowercase database name (e.g., . test), connect to CDB using 'sq', connect to each PDB,
# and output only PDB name and used size (in GB) in a single-node RAC database.
# Includes debugging to diagnose environment setup failure.
# Assumptions:
# - Run as the oracle user with access to sq (alias/script for sqlplus / as sysdba).
# - Sourcing the lowercase database name (e.g., . test) sets ORACLE_SID, ORACLE_HOME, and PATH.
# - Databases and instances are running; offline databases/PDBs are skipped.
...
for db in $dbs; do
  ...
  # Attempt to set Oracle environment by sourcing the lowercase database name
  if ! . $db >/dev/null 2>&1; then
    echo "DEBUG: Database $db: Failed to source environment script (e.g., . $db)." >&2
    continue
  fi
  
  # Debug: Log environment variables after sourcing
  echo "DEBUG: Database $db: ORACLE_SID=$ORACLE_SID, ORACLE_HOME=$ORACLE_HOME, PATH=$PATH" >&2
